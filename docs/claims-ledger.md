...
| El guardia distingue un archivo que se abre de un nombre que solo se menciona: deniega `cat .env`, un operando entre comillas, una redirección y una asignación; permite `echo`, `printf`, `-m`/`--message`, cuerpos de heredoc y here-strings que nombren el archivo. | 42 casos ejecutados contra el hook con payloads reales, sobre `scripts/agent-hooks/` y sobre la plantilla de la skill; 42/42 — ejecutados en la sesión, **no versionados** | alta | confirmada |
| Los tres comandos que el guardia bloqueó por error en la sesión del 2026-08-25 hoy pasan. | reejecutados verbatim contra el hook | alta | confirmada |
