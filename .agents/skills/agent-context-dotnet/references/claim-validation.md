...
> This procedure is intentionally duplicated in every skill that generates docs (skills ship
...
- The 3–6 most-run developer commands — especially the run command when Aspire is involved and
  the test command when a non-default runner is configured.
- Key domain entities and their relationships.
- Package versions you named, and whether versions are managed centrally.
- The user-supplied non-obvious rules / invariants (project-layering rules, DI lifetimes).
...
- **Confidence**:
  - `high` — read verbatim/near-verbatim from a file (e.g. a version pinned in a manifest).
  - `medium` — inferred from a single weak or indirect signal.
  - `low` — guessed, or supplied by the user but unverified against the repo.

Example: `"The API targets .NET 10."` → source `src/Api/Api.csproj:6`, confidence `high`.

## Step 3 — Flag ambiguity

Mark a claim **ambiguous** when:

- More than one interpretation is plausible (e.g. two projects pin different target frameworks; an
...
- For the **top few binary, high-stakes** confirmations, use `AskUserQuestion` (e.g. "Is the
  database PostgreSQL or SQL Server?"). Offer the candidate readings as options.
...
- Re-check that corrected claims didn't break cross-doc consistency (e.g. a version named in both
  `architecture.md` and `dotnet.md`).
...
Generated by the `agent-context-dotnet` skill. Records the load-bearing factual claims in the docs,
their source in the repository, and whether a human confirmed them. Re-run the skill to refresh.

| Claim | Source | Confidence | Status |
|---|---|---|---|
| The API targets .NET 10. | `src/Api/Api.csproj:6` | high | confirmed |
| Persistence is PostgreSQL via Npgsql. | inferred | medium | corrected → SQL Server |
| Migrations run in CI before deploy. | user | low | TODO: verify |
```

**Spanish header** (when output language is `es`):
...
|---|---|---|---|
| La API usa .NET 10. | `src/Api/Api.csproj:6` | alta | confirmada |
| La persistencia es PostgreSQL vía Npgsql. | inferido | media | corregida → SQL Server |
| Las migraciones corren en CI antes del deploy. | usuario | baja | TODO: verificar |
```
...
- The repository is the source of truth. Never invent a version, package, endpoint, or schema
