...
describe('StockBadge', () => {
  ...
  it('Render_StockTen_ShowsRunningLowAtTheUpperBoundary', async () => {
    ...
    expect(badge.classList).toContain('bg-neutral');
    expect(badge.classList).toContain('text-primary-strong');
    expect(badge.classList).toContain('border-primary');
  });
  ...
  it('Render_StockZero_ShowsOutOfStock', async () => {
    ...
    expect(badge.classList).toContain('border-destructive');
  });
  ...
  it('Render_StockChangesFromZeroToAvailable_SwapsSymbolTextAndClasses', async () => {
    ...
    expect(available.classList).toContain('text-text-muted');
    expect(available.classList).not.toContain('text-destructive');
  });
});
